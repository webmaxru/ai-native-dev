---
name: "ARD Registry Builder: Update Skill"
description: "Refresh the skills/ard-registry-builder skill from user-supplied textual updates, attached documents, and the current Agentic Resource Discovery (ARD)documentation. Use when ARD specification may have changed and the skill, references, assets, or examples need to be reconciled."
...
Update the `skills/ard-registry-builder` skill in this workspace using the latest available inputs and any textual updates the user supplies.
...
3. Built-in prompt URLs fill gaps not covered by higher-priority sources, using `https://github.com/ards-project/ard-spec/tree/main/spec` as the primary URL source and its official linked reference and guide pages as secondary built-in sources.
...
- Use the built-in prompt URLs to fill gaps that are not covered or are less detailed in the higher-priority sources.
- Prefer the official ARD documentation pages that are most specific to the topic being updated when no higher-priority source overrides them.
...
- Read the current skill files under [skills/ard-registry-builder](../../skills/ard-registry-builder/), including `SKILL.md`, `references/`, `assets/`, and `scripts/`.
...
2. Extract only material changes that affect the skill
3. Compare the extracted changes against the current files in `skills/ard-registry-builder`.
...
```bash
python .github/skills/skill-creator/scripts/validate-metadata.py --name "ard-registry-builder" --description ">-
